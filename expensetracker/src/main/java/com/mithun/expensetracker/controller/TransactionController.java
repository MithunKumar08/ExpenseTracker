...
public class TransactionController {
    ...
    @PostMapping("add")
    public ResponseEntity<UserTransaction> makeTransaction(@RequestBody @Valid UserTransaction transaction, @AuthenticationPrincipal User user){
        try {
    ...
    public ResponseEntity<List<UserTransaction>> getAllTransactions(@AuthenticationPrincipal User user){
        System.out.println(user.getUsername());
        return new ResponseEntity<>(transactionService.getAllTransactions(),HttpStatus.OK);
    }

    @GetMapping("getUserTransaction")
    public ResponseEntity<List<UserTransaction>> getUserTransaction(@AuthenticationPrincipal User user){
